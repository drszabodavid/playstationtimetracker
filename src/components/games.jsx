...
class Games extends Component {
    ...
    handleRecord = async (game, time) => {
        this.updateTimeBeforeDatabaseReturn(game, time);
    ...
    updateTimeBeforeDatabaseReturn = (game, time) => {
        const games = [...this.state.games];
        const index = games.indexOf(game);
        games[index] = {...games[index]};
        let timeScored = time * 10000;
        let hour = Math.floor(timeScored / 1000 / 60 / 60);
        games[index].timeSpent = games[index].timeSpent + hour;
        games[index].remainingTime = games[index].gameplayMain - games[index].timeSpent <= 0 ? 0 : games[index].gameplayMain - games[index].timeSpent;
        this.setState({games});
    };
